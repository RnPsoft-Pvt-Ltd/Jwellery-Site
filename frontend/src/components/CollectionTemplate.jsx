...
const CollectionTemplate = ({ 
  ...
  useEffect(() => {
    const fetchCollection = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`https://api.shopevella.com/v1/collections/${collectionId}`);
  ...
  const allVariants = transformProductsToVariants(collection?.products);
  const filteredVariants = allVariants
    ...
    .sort((a, b) => {
      if (sort === "price-low-high") return a.base_price - b.base_price;
      if (sort === "price-high-low") return b.base_price - a.base_price;
      return 0;
    })
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-900">{collection?.name || title}</h1>
