...
const SalesTemplate = ({ 
  ...
  const allVariants = category?.sale_products?.flatMap((item) => 
    item.product.variants?.map(variant => ({
      ...variant,
      productId: item.product.id,
      productName: item.product.name,
      basePrice: item.product.base_price,
      totalPrice: calculateVariantPrice(item.product.base_price, variant.price_modifier),
      description: item.product.description,
      images: item.product.images // We'll use variant-specific images if available
    })) || []
) || [];
